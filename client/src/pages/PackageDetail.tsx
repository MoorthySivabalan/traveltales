...
import {
  ArrowLeft, MapPin, Clock, Star, CheckCircle,
  ChevronDown, ChevronUp, Copy, Sparkles, Calendar
} from 'lucide-react'
...
const PackageDetail = () => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-950">
        <div className="text-center">
          <h2 className="font-serif text-2xl text-navy dark:text-white mb-4">Package not found</h2>
  ...
  const handleEditCopy = () => {
    ...
    console.log('Edit copy of', pkg.name)
  }
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10">
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-wrap gap-2 mb-3">
              {pkg.tags.map(tag => (
                <span key={tag} className="text-xs font-medium bg-brand text-white px-2.5 py-1 rounded-full">
                  {tag}
                </span>
              ))}
            </div>
            <h1 className="font-serif text-3xl md:text-5xl text-white mb-2">{pkg.name}</h1>
            <div className="flex flex-wrap items-center gap-4 text-gray-300 text-sm">
              <span className="flex items-center gap-1">
                <MapPin className="w-4 h-4 text-accent" />{pkg.state}
              </span>
              <span className="flex items-center gap-1">
                <Clock className="w-4 h-4 text-accent" />{pkg.duration} Days
              </span>
              <span className="flex items-center gap-1">
                <Star className="w-4 h-4 fill-accent text-accent" />4.8 Rating
              </span>
            </div>
          </div>
        </div>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-medium text-navy dark:text-white text-sm text-left">
                            {dayLabel}
                            {isOpen ? '' : ` — ${dayContent.slice(0, 50)}${dayContent.length > 50 ? '...' : ''}`}
                          </span>
                        </div>
                        {isOpen
          ...
          <div className="space-y-5">
            <div className="sticky top-20 space-y-5">
              ...
              <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 border border-gray-100 dark:border-gray-800 shadow-sm">
                ...
                <div className="flex gap-2 mb-5 p-1 bg-gray-100 dark:bg-gray-800 rounded-xl">
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${tier === 'economy'
                  ...
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${tier === 'premium'
                      ? 'bg-white dark:bg-gray-700 text-navy dark:text-white shadow-sm'
                      : 'text-gray-500 dark:text-gray-400'}`}
                  >
                    Premium
                ...
                <div className="space-y-3 mb-5">
                  <div className="flex items-start justify-between gap-2 p-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                    <div>
                      <p className="text-xs text-gray-400 mb-0.5">Hotel / Stay</p>
                ...
                <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-5 pb-5 border-b border-gray-100 dark:border-gray-800">
                  ...
                  <span>{pkg.duration} Days · {pkg.region}</span>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-3 border border-gray-200 dark:border-gray-700 hover:border-brand dark:hover:border-blue-400 text-gray-600 dark:text-gray-300 hover:text-brand dark:hover:text-blue-400 rounded-xl text-sm font-medium transition-all"
                  >
              ...
              <div className="bg-navy dark:bg-gray-900 rounded-2xl p-5 border border-gray-800">
                <h4 className="text-white font-medium text-sm mb-3">Trip Summary</h4>
                <ul className="space-y-2">
                  ...
                  <li className="flex justify-between text-sm">
                    ...
                    <span className="text-white font-medium">{pkg.tags[0]}</span>
                  </li>
                </ul>
              </div>
